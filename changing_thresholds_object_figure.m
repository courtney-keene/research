...
upzbot=nan(53727,1);
updnorm=nan(53727,1);
upzbot_norm=nan(53727,1);
upzx_norm=nan(53727,1);
upz_norm=nan(53727,1);
upzxn=nan(53727,1);
...
for ff=1:length(files)
    ...
    if filetime>=(startday+(19/24)) && filetime<endday+(21/24) %&& hour>=15 && hour<23 % thr-5  && hour<22%thr+4
        ...
        if size(W,2)>1
            ...
            spdnow=wspdmaster(whidx,wtidx); %pa
...
if numel(regions)>0
    for rr=1:length(regions)
        if regions(rr).Area>size_threshold %must be at least 100 pixels in the area
            ...
            [zbot,zidxn]=min(regions(rr).ConvexHull(:,2));
            ...
            upz_norm(draft_count)=(0.5.*(upzc(draft_count)+upzx(draft_count)))./upZi(draft_count);
            upzxn(draft_count)=upzx(draft_count)./upZi(draft_count);

            updnorm(draft_count)=(upzx(draft_count)-upzbot(draft_count))./upZi(draft_count);
            upzbot_norm(draft_count)=upzbot(draft_count)./upZi(draft_count);
            upzx_norm(draft_count)=upzx(draft_count)./upZi(draft_count);
